# Practice Exam: All 12 Sample Questions

All questions and explanations are reproduced from the official exam guide (v0.1).
Each question is cross-referenced to the code file that demonstrates the concept.

---

## Scenario: Customer Support Resolution Agent

### Question 1 ★★★ (Domain 1, 2 — Most Important)
...
**Code Reference:** `scenarios/scenario-1-customer-support/agent.py` → `WorkflowState` class, `execute_tool()` prerequisite checks
**Also see:** `domain-1-agentic-architecture/1_4_workflow_enforcement.py`
...
**Code Reference:** `domain-2-tool-design-mcp/2_1_tool_descriptions.py` → `GOOD_TOOLS` vs `BAD_TOOLS`

---

### Question 3 (Domain 5, 4)
...
**Code Reference:** `domain-5-context-reliability/5_2_escalation_patterns.py` → `ESCALATION_SYSTEM_PROMPT`
**Also see:** `scenarios/scenario-1-customer-support/agent.py` → `SYSTEM_PROMPT` with escalation criteria
...
**Code Reference:** `domain-3-claude-code/3_2_commands_skills/commands/review.md`
**Also see:** `.claude/commands/review.md` in this repo's own config
...
**Code Reference:** `domain-3-claude-code/3_4_plan_vs_direct.md` → decision matrix
...
**Code Reference:** `domain-3-claude-code/3_3_path_rules/tests.md` → `paths: ["**/*.test.*"]`
...
**Code Reference:** `domain-1-agentic-architecture/1_2_coordinator_subagent.py` → `coordinator()` function, `antipattern_narrow_decomposition()`

---

### Question 8 ★ (Domain 1, 5)
...
**Code Reference:** `domain-5-context-reliability/5_3_error_propagation.py` → `make_structured_error_for_coordinator()`
**Also see:** `domain-2-tool-design-mcp/2_2_structured_errors.py`
...
**Code Reference:** `domain-2-tool-design-mcp/2_3_tool_distribution.py` → `demonstrate_scoped_cross_role_tool()`
...
**Code Reference:** `domain-3-claude-code/3_6_ci_cd/.github/workflows/claude-review.yml`
**CLI:** `claude -p "..." --output-format json --json-schema schema.json`
...
**Code Reference:** `domain-4-prompt-engineering/4_5_batch_processing.py` → `show_batch_vs_sync_decision()`

---

### Question 12 (Domain 1, 4)
...
**Code Reference:** `domain-4-prompt-engineering/4_6_multi_pass_review.py` → `multi_pass_code_review()`
**Also see:** `domain-1-agentic-architecture/1_6_task_decomposition.py` → `prompt_chain_code_review()`

---

## Score Analysis

If you got these wrong, prioritize:

| Question | Wrong Answer → Study This |
|----------|--------------------------|
| Q1 | Programmatic enforcement → `1_4_workflow_enforcement.py` |
| Q3 | Escalation criteria → `5_2_escalation_patterns.py` |
| Q5 | Plan mode decision → `3_4_plan_vs_direct.md` |
| Q6 | Path rules with glob → `3_3_path_rules/tests.md` |
| Q7 | Coordinator decomposition → `1_2_coordinator_subagent.py` |
| Q8 | Structured error context → `5_3_error_propagation.py` |
| Q9 | Scoped cross-role tools → `2_3_tool_distribution.py` |
| Q11 | Batch API limitations → `4_5_batch_processing.py` |
